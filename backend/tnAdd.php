...
if($requestBody['id_testpoint'] ){
    $id_testpoint = $requestBody['id_testpoint'];

    $actual_thickness=$requestBody['actual_thickness'];
    $inspection_date=$requestBody['inspection_date'];

    $sql = "INSERT INTO THICKNESS(inspection_date,actual_thickness,id_testpoint)
            VALUES ('$inspection_date',$actual_thickness,$id_testpoint);";
    mysqli_query($con, $sql) or trigger_error("Query Failed! SQL: $sql - Error: ".mysqli_error($con), E_USER_ERROR);
    echo 'add Thickness succcessful';
}
else {
